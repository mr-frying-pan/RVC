...
/**
 * Error signifying that low bits of an immediate would be lost if it were compiled.
 */
class LowBitsLostError : public SyntaxError {
private:
  std::uint32_t imm; /**< value used as an operand */
  std::uint8_t lowBit; /**< highest bit of imm which would be lost */
public:
  LowBitsLostError(std::uint32_t imm, std::uint8_t lowBit) noexcept;
  std::string to_string() const noexcept override;
};


/**
 * Error signifying that high bits of an immediate would be lost if it were compiled.
 */
class HighBitsLostError : public SyntaxError {
private:
  std::uint32_t imm; /**< value used as an operand */
  std::uint8_t highBit; /**< lowest bit which woud be lost */
public:
  HighBitsLostError(std::uint32_t imm, std::uint8_t highBit) noexcept;
  std::string to_string() const noexcept override;
};
